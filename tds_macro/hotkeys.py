...
class HotkeyManager:
    ...
    def start(self) -> bool:
        ...
        try:
            ...
            mapping = {
                _to_pynput_combo(self.config.panic_hotkey): self._on_panic,
                _to_pynput_combo(self.config.pause_hotkey): self._on_pause,
                _to_pynput_combo(self.config.start_hotkey): self._on_start,
                _to_pynput_combo(self.config.mark_sync_hotkey): self._on_mark_sync,
            }
            self._listener = keyboard.GlobalHotKeys(mapping)
            self._listener.start()
            installed = True
        except Exception:
            # No pynput / no permission: tests + non-mac drive events directly.
            self._listener = None
