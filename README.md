...
|---|---|
| **Strat file** + **Browse…** | pick (or name) the `.strat.json` to record into / play |
...
Give the macro a private-server link and it joins by **opening the link** — at start and
after every recovery/loop restart — instead of clicking through the lobby:
